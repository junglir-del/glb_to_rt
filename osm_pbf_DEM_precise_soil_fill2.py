...
def lonlat_bbox_polygon(bbox):
    min_lon, min_lat, max_lon, max_lat = bbox
    return box(min_lon, min_lat, max_lon, max_lat)


def geometry_intersects_bbox(coords, bbox):
    if len(coords) < 2:
        return False

    bbox_polygon = lonlat_bbox_polygon(bbox)
    ...
    return (not geometry.is_empty) and geometry.intersects(bbox_polygon)


def clipped_lonlat_polygons(coords, bbox):
    if len(coords) < 4:
        return []
    ...
    clipped = polygon.intersection(lonlat_bbox_polygon(bbox))
    rings = []
...
def opentopography_ssl_context(insecure_ssl=False):
    if insecure_ssl:
        print("WARNING: OpenTopography DEM download is using insecure SSL verification.")
...
def download_opentopography_dem(bbox, dem_type, output_path, api_key, insecure_ssl=False):
    if not api_key:
        raise ValueError(
    ...
    ssl_context = opentopography_ssl_context(insecure_ssl)
...
class OsmCollector(osmium.SimpleHandler):
    ...
    def way(self, way):
        ...
        if len(coords) < 2:
            return
        if not geometry_intersects_bbox(coords, self.bbox):
            return
    ...
    def relation(self, relation):
        ...
        for ring in join_way_segments(outer_segments):
            if not geometry_intersects_bbox(ring, self.bbox):
                continue
...
def triangulate_projected_polygon(polygon, origin_lon, origin_lat, dem, z_offset):
    ...
    for x, y in vertices_2d:
        ...
        height = dem.sample(lon, lat, default=0.0) if dem else 0.0
        positions.append([x, height + z_offset, -y])
...
def build_road_mesh(coords, width, origin_lon, origin_lat, y=0.03, dem=None, clip_bbox=None):
    xy = [mercator_xy(lon, lat, origin_lon, origin_lat) for lon, lat in coords]
    ...
    if clip_bbox is not None:
        ...
        road_geometry = line.buffer(half_width, cap_style=2, join_style=2)
        road_geometry = road_geometry.intersection(
            projected_bbox_polygon(clip_bbox, origin_lon, origin_lat)
        )
        ...
        positions = []
        indices = []
        for polygon in iter_polygons(road_geometry):
...
def main():
    ...
    parser.add_argument(
        "--insecure-dem-download",
        action="store_true",
        help="Disable SSL certificate verification for OpenTopography DEM download. Use only as a temporary workaround.",
    )
    ...
    if args.download_dem:
        ...
        download_opentopography_dem(
            ...
            insecure_ssl=args.insecure_dem_download,
        )
    ...
    for building in collector.buildings:
        ...
        clipped_building_coords = clipped_lonlat_polygons(building["coords"], bbox)
        for part_index, building_coords in enumerate(clipped_building_coords):
            part_name = name if len(clipped_building_coords) == 1 else f"{name}_part_{part_index + 1}"
            ...
            metadata["clipped_to_bbox"] = True
            metadata["clip_part_index"] = part_index
            metadata["clip_part_count"] = len(clipped_building_coords)
            metadata["gltf_node_index"] = len(writer.nodes)
            metadata["vertex_count"] = len(positions)
            metadata["triangle_count"] = len(indices) // 3
    ...
    for area in collector.areas:
        ...
        clipped_area_coords = clipped_lonlat_polygons(area["coords"], bbox)

        for part_index, area_coords in enumerate(clipped_area_coords):
            part_name = name if len(clipped_area_coords) == 1 else f"{name}_part_{part_index + 1}"
            ...
            metadata = make_surface_metadata(area, part_name, feature_type)
            metadata["clipped_to_bbox"] = True
            metadata["clip_part_index"] = part_index
            metadata["clip_part_count"] = len(clipped_area_coords)
            metadata["gltf_node_index"] = len(writer.nodes)
            metadata["vertex_count"] = len(positions)
            metadata["triangle_count"] = len(indices) // 3
